...
> **2026-08-10 事实源与工具兼容收敛说明：** [ACT Protocol 官网](https://www.act-protocol.com/)已经取代内部维护文档成为公开协议事实源；语雀链接仅作为 Candidate 演进证据，外部开发者无需访问。A402 Schema `$id` 已改为可随仓库获取位置解析的相对 URI，非幂等请求和 `Payment-Validation` 的关键跨字段约束也已进入 JSON Schema。支付宝 AI 按量付费[公开接入指南](https://aipay.alipay.com/docs/ai-receive/MACHINE_PAY.html)当前可访问，并明确公开两类 Header 和验凭/履约 API 名。本文后续未逐段改写的旧来源、旧 Schema 和旧产品可核实性结论仅保留为历史发现，不代表当前状态。
>
> **2026-08-03 收敛说明：** 当时以完成版[《支付服务域》](https://yuque.antfin.com/hknzlf/fvle20/dve0b9g2u1t3cs33)形成 Candidate 快照。A402 Candidate 决策、三类 Header Schema、fixtures、validator、十一类错误和状态转移已经完成；本文后续未逐段改写的“Core Schema 缺失”“九项决策待定”等内容仅保留为历史发现，不代表当前状态。当前状态以[分级发布门禁](../release-readiness.json)和[项目修订状态](../revision-status.md)为准。
> **2026-08-06 CID 收敛说明：** ACT 2.1 官网尚未更新，商业交互域暂以观岳维护的[《商业交互域》](https://yuque.antfin.com/hknzlf/fvle20/vn8s21q6cc7pmmon)为事实源。维护者确认只有黄色高亮的“安全考虑”是本轮修改；CID 四组件等其余内容属于既有基线。仓库已补齐完整基线，并将能力声明来源校验、关键字段一致性和失败关闭作为修订增量保护。正式 Schema、字段冲突和签名/版本规则继续 Pending。公开版权与贡献政策见[版权主体与贡献政策记录](../decisions/legal-and-contribution-policy.md)。
> **2026-08-06 四域收敛说明：** 仓库进一步对齐[《委托授权域》](https://yuque.antfin.com/hknzlf/fvle20/fzy9k8wgpfgf6umt)、[《典型场景与业务流程》](https://yuque.antfin.com/hknzlf/fvle20/yz11ev27zqtcgeeu)、[《可信存证》](https://yuque.antfin.com/hknzlf/fvle20/gcgkq98a1d7z63yz)和新增[《信用关联》](https://yuque.antfin.com/hknzlf/fvle20/uggov9ooo4g5tukx)。四域语义 Candidate 现已齐备，PSD 仍是一键接入主轴。ADD、CID、TSD 没有被补造正式 wire Schema；仓库也不实现来源未公开的 ACT Trust Chain 或信用模型。黄色增量与首次基线补齐已分别追踪。
> **2026-08-11 发布治理收敛说明：** 支付宝产品事实已复核至官网 `2026-08-07 21:51:27` 版，正文接入语义未发现变化；仓库已有每日漂移检查。公开文件不再依赖 `governance/internal`，公开快照可独立重建并通过检查；Showcase 已进入 Release Manifest；私密安全渠道已确定为 [AntSRC](https://security.alipay.com/)，当前不启用额外贡献协议或 DCO sign-off 前置。公开发布只剩公开仓库地址和最终干净提交需要真实外部配置。
...
**结论：Core / Binding / Product Profile / Quickstart 的分层架构合理。四域语义 Candidate 已齐备，当前可执行主轴仍是“完成版支付服务域协议 + 支付宝产品事实”；修订分析只作历史追溯，典型场景负责帮助开发者选择所需授权与信任能力。**
...
4. **四域语义范围已补齐，机器契约边界保持克制** —— ADD 三组件、CID 四组件、PSD 六组件、TSD 两个子篇十组件及四类典型场景均有 Candidate 入口；除 A402 外的正式 Schema、Trust Chain 和信用服务仍是显式 Pending，而不是假实现。

治理、第二 Product Profile 和正式 Conformance 仍是发布成熟度问题，但不应取代“PSD 完整性 + L1/A402 一键接入”的近期主任务。版权、双许可证和入站贡献条款已确认。
...
3. **不能用工程修订替代域正文没有规定的决定。** A402 Candidate 已通过显式 Candidate 决策固定 `method_id`、字段必填性、幂等键和错误对象；CID 已同步单/双向协商及来源校验语义，但正式 Schema、字段冲突、认证/重定向、缓存/完整性和 L2/L3 扩展仍保持 Pending，不由 Quickstart 或支付宝产品事实反向决定。
4. **测试层级不等于规范强度。** `documentation` / `integration` / `end-to-end` 描述验证环境，不能机械映射为 RFC 2119 的 MAY / SHOULD / MUST。
5. **基金会托管、npm 发布和第二 Product Profile 需要新授权或外部协调。** 法务发布条件已确认；其余事项不能由一次文档修订自行宣布完成。
...
| R-CID 商业交互域同步 | 登记语雀 CID 来源；同步四组件、核心对象、支付能力协商和购物车确认 | **Completed as Candidate；wire decisions pending** | 自动检查阻止来源、组件和 Pending Decision 丢失 |
| R-ADD 委托授权域同步 | 登记 ADD 来源；同步意图、ISR、IAC 签发和生命周期，并单独记录黄色字段修订 | **Completed as semantic Candidate；wire decisions pending** | 自动检查阻止来源、三组件、字段修订和 Pending Decision 丢失 |
| R-SCENARIO 典型场景 | 建立用户在场、平台/专属 Agent 定向委托和自主委托四类跨域流程 | **Completed as Non-normative guide** | 场景冲突时域正文优先，自动保护 A402 独立性和异步 TSD 边界 |
| R-TSD 信任服务域同步 | 区分可信存证与信用关联两个子篇，登记十组件、来源指纹和机器契约缺口 | **Completed as semantic Candidate；wire/implementation pending** | 不把产品日志、链服务或信用结果误称为 TSD 实现 |
...
| R5 法务和中立治理 | 确认版权主体、许可证、入站贡献条款、委员会和可能的基金会路径 | **Legal publication gate completed；governance evolution pending** | 当前无额外签署前置；后续按需演进委员会或基金会路径 |
...
| R-RELEASE 分级收口 | ACT Candidate、Profile Preview、Sandbox Verified、SEP Candidate 使用独立门禁 | **Implemented；two publication gates remain** | 公开远端和干净发布 Commit 通过；AntSRC 安全渠道已完成 |
...
> 2026-08-03 状态：**已解决为 Candidate**。Core Schema 位于 `specs/2.1/a402/schemas/`；支付宝产品字段继续由独立 Profile Preview Schema 管理。
...
`specs/2.1/a402/assertions/a402-core-assertions.json` 的 `statement` 是自然语言句子，无 JSONPath、无 schema 引用、无输入输出契约；`source` 锚点全部失真（指向 `#a402-的独立性`，实际标题是 `## 1. 独立性`）。`assertion-catalog.schema.json:41` 把 ID 锁死在 `^A402-CAND-[0-9]{3}$`，无法扩展到 CID/INS/DEL/AUP。
...
- **本地 Golden Path 定位清晰**：`README.md:8-14` 一条 `npm run local` 零凭证跑通，`code/examples/alipay/end-to-end-402/README.md:7-17` 讲清「只验证假 Proof 不交付」安全边界。
- **仓库/官网责任边界表**（`code/examples/alipay/README.md:26-39`）把「本仓库负责 / 官网负责」二分干净，是新人最友好的资产。
- **资产五分类定义**（Scenario / Demo / Quickstart / Reference Implementation / Conformance Suite，`README.md:109-121`）有效区分「能跑」与「能声明合规」。
- **Profile 状态标签自检表**（`integrations/profiles/alipay-ai-pay/README.md:42-51`：PUBLIC-FACT / PRODUCT-REVIEW / PROTOCOL-PENDING / VALIDATION-PENDING）。
...
**D5. 仓库卫生事故级问题：**
- `.gitignore` 漏了 `.tmp/`、`output/`、`target/`（Maven 构建产物）。当前虽未追踪，但下一次 `git add .` 极易误提交 `.tmp/aipay-two-slide-ppt/`、`output/AIPay_AI付_AI收_两页介绍初稿.pptx`、`code/examples/.../target/*.jar`。
...
- Quickstart 残留「大会 Demo」措辞（`code/examples/alipay/agent-payment/README.md:70-74`），属内部演示脚本改写的场景化残留。

> 本轮状态：`.gitignore` 和自动检查已覆盖 `.tmp/`、`output/`、`target/`；公开接入与证据文档已改用角色和公开评审记录。修订来源、历史决策和发布项目文档仍保留事实性责任人。

**D6. 法务与贡献条款：** 已确认 `Copyright (c) 2026 Ant Group Co., Ltd.`、文档 CC BY 4.0、代码/Schema Apache 2.0；根 `LICENSE`、`NOTICE` 与贡献边界已经落地，法务发布门禁通过。2026-08-11 进一步确认当前不启用额外贡献协议或 DCO sign-off，外部 PR 按普通权利声明、来源检查和维护者评审处理。
...
|---|---|---|---|---|---|
| 仓库结构 | spec+docs monorepo，SDK/samples 兄弟 repo | 多仓：spec + 10 SDK + servers + inspector + conformance | polyglot monorepo（TS/Py/Go/Java + specs + contracts） | monorepo 多语言 + 内嵌 MCP server | 六主干单仓：specs / docs / integrations / code / governance / scripts；SDK 不虚设空目录 |
...
> 2026-08-11 状态：P0-0、P0-1、P0-2、P0-3、P0-4 已完成到 Candidate；P0-3 已确认 `Ant Group Co., Ltd.`、CC BY 4.0 / Apache 2.0，且当前无额外签署前置。AntSRC 安全渠道已经配置；公开发布继续受公开仓库目标和干净发布快照阻塞，支付宝沙箱不阻塞 ACT Candidate/Profile Preview。
...
| **P0-3** | **落地法务**：版权主体、双许可证和入站贡献条款已确定；当前无额外签署前置 | D6 | 发布门禁 Done；外部贡献按许可证、权利声明和维护者评审处理 |
...
| **P2-4** | **引入 ADR 机制**：先提供模板；只有真实决定被接受后，才把 A402 编码、版本和兼容选择记录为 `governance/decisions/adr-00x`，不得用占位 ADR 冒充决定 | §4.2 A2A 借鉴 | 模板、编号规则和索引存在；每份 ADR 都链接实际讨论与批准记录 |
...
**退出条件**：P0-0、P0-2、P0-3、P0-4 完成；P0-1 有决策记录和 canonical-source 方案。
**产出**：内容可作为 `ACT 2.1 Candidate Working Draft / Non-normative`；完成公开远端和发布快照后即可实际公开，且不可声称正式合规。AntSRC 安全渠道已完成；Product Profile Preview Schema 不能替代 Core Schema。
