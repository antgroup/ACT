...
July Preview 的候选内容和本地工程验证已经准备完成，但当前不是已公开发布的版本。该页是历史快照；当前发布状态以[机器可读门禁](../release-readiness.json)为准，AntSRC 安全渠道已经配置，仍待公开远端和干净发布快照。
...
支付宝产品变化见[本次产品事实审计](../../integrations/profiles/alipay-ai-pay/sources/audits/2026-07-31-product-update.md)。
...
July Preview 当时的阻塞项及当前处理结果：

1. 版权主体、文档/代码许可证分配和入站贡献策略：已完成；
2. 私密安全披露渠道：已配置 [AntSRC](https://security.alipay.com/)；
3. 公开代码托管远端：仍待确认；
4. 审查当前重构变更，生成干净 Commit，并从该 Commit 重新运行 CI：仍待完成；
5. 只有当前门禁通过后，才创建 Preview Tag 和公开 Release。
