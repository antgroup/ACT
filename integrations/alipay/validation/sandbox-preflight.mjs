...
const DEFAULT_ENV_FILE = resolve(HERE, "../seller-java/.env");
...
const CNY_AMOUNT = /^(?:0|[1-9][0-9]*)(?:\.[0-9]{1,2})?$/u;

function validOfficialAmount(value) {
  if (!CNY_AMOUNT.test(value || "")) return false;
...
export function inspectSandboxReadiness({
  ...
  if (!envFileExists) {
    ...
    if (env.ALIPAY_AMOUNT && !PLACEHOLDER.test(env.ALIPAY_AMOUNT)
        && !validOfficialAmount(env.ALIPAY_AMOUNT)) {
      failures.push("ALIPAY_AMOUNT must be between 0.01 and 50.00 CNY with at most two fraction digits");
    }
    const currency = (env.ALIPAY_CURRENCY || "CNY").toUpperCase();
    if (currency !== "CNY") {
      failures.push("ALIPAY_CURRENCY must be CNY for the current metered-payment product");
    }
    if (
      !secretConfigured(
        env,
    ...
    checks: {
      ...
      seller_required_fields:
        ...
        REQUIRED_SELLER_FIELDS.every(
          ...
          && (env.ALIPAY_CURRENCY || "CNY").toUpperCase() === "CNY",
