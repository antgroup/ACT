...
test("rejects product prices outside the current official CNY registration range", () => {
  const result = inspectSandboxReadiness({
    envFileExists: true,
    envText: COMPLETE_ENV.replace("ALIPAY_AMOUNT=0.01", "ALIPAY_AMOUNT=50.001")
      + "\nALIPAY_CURRENCY=USD\n",
    fileExists: () => true,
    commands: { npm: true, alipayBot: true, java: true, maven: true },
  });

  assert.equal(result.status, "not-ready");
  assert.ok(result.failures.some((failure) => failure.startsWith("ALIPAY_AMOUNT must")));
  assert.ok(result.failures.some((failure) => failure.startsWith("ALIPAY_CURRENCY must")));
  assert.equal(result.checks.seller_required_fields, false);
});
