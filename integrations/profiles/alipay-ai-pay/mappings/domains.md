...
> 状态：Profile `0.9-preview.1` / Non-normative  
> 协议事实源：[ACT Protocol 官网](https://www.act-protocol.com/)；最后核对 2026-08-10
> 产品事实基线：支付宝 AI 付公开官网与官方接入文档  
...
当前 Core 映射以 ACT Protocol 官网为协议事实源；仓库 Candidate 与官网发布存在差异时，必须登记并评审，不能由 Product Profile 猜测或覆盖 Core。四域语义 Candidate 已形成公开快照，开源可执行接入仍以 PSD 为主：
...
| [PSD：Payment Services Domain](https://www.act-protocol.com/documentation/payment) | `PSD-PMT-BND`、`PSD-AGT-SUB`、`PSD-PAY-INS`、`PSD-PAY-DEL`、`PSD-PAY-AUP`、`PSD-PAY-A402` | 支付方式如何可用；以何种授权级别发起、证明和验证支付 |
| [TSD：Trust Services Domain](https://www.act-protocol.com/documentation/trust) | `TSD-ATT-*` 五组件 + `TSD-CRD-*` 五组件 | 如何形成可验证事件链、存证、核验和争议处理；如何在受限授权下引用关联主体信用 |

完成版《支付服务域》把 PSD 的“支付场景”与“接入协议”分开：`PSD-PAY-INS`、`PSD-PAY-DEL`、`PSD-PAY-AUP` 分别描述 L1/L2/L3 场景，`PSD-PAY-A402` 独立描述 HTTP 402 支付交互。仓库中的公开文本仍处于 Candidate / Non-normative，因此不能据此声明正式 Conformance；支付宝产品实现仍以公开官网为准。

因此，开发者的阅读顺序应当是：先通过[典型场景](../../../../specs/2.1/scenarios.md)确认用户是否在场和授权级别，再确认自己建设买方还是卖方能力，最后进入支付宝官网完成真实产品接入。需要声明端到端兼容时，必须同时验证两侧能力。
...
这个表描述职责覆盖，不表示支付宝产品实现了 ACT 每个组件的完整标准接口。具体状态使用以下标签：
...
| `CID-PCA-NEG` | 识别对方支持支付宝，并选择官方支付能力 | `CURRENT` / `PROTOCOL-PENDING` | CID 已定义单向声明与双向协商语义；正式 Schema 和支付宝产品字段映射待确认 |
...
| 候选 `PSD-PAY-A402` | 处理 402 支付要求、支付凭证和资源请求恢复 | `CURRENT` | 完成版协议已明确独立组件和三类 Header；公开规范状态仍为 Candidate |
...
| `TSD-ATT-EVT` 等 | 将支付结果与意图、订单和履约证据关联 | `HOST` / `PROTOCOL-PENDING` | TSD 语义 Candidate 已明确；支付宝交易记录仍不自动等同于 ACT 链下记录、链上锚点或核验结论 |
...
ACT v2.1 完成版协议将完整 402 `Payment-Needed` / `Payment-Proof` / `Payment-Validation` 框架建模为独立的 `PSD-PAY-A402`，供 INS、DEL、AUP 引用。首期因此按“L1 `PSD-PAY-INS` + 候选 `PSD-PAY-A402` + 官方 Skill/CLI Binding + Alipay Profile”组织，但在仓库候选进入正式治理发布前不声明正式合规。
...
| `CID-PCA-NEG` | 服务声明支付宝支付方式，Agent 选择可用方式 | `CURRENT` / `PROTOCOL-PENDING` | CID 已固定协商结果语义；402 产品字段、能力地址和正式 Schema 映射待确认 |
| `CID-CART-CFM` | 确认资源、金额、币种、商户订单和收款方 | `CURRENT` | 支付请求不要求传完整购物车；A402 账单携订单/资源等必要关联标识 |
| 候选 `PSD-PAY-A402` | `402 Payment Required`、`Payment-Needed`、`Payment-Proof`、验款和交付 | `CURRENT` | 完成版协议定义独立组件；支付宝字段与 RSA2 规则属于 Alipay Profile |
...
| `TSD-ATT-EVT` 等 | 支付完成、履约完成及关联证据 | `HOST` / `PROTOCOL-PENDING` | TSD 事件与双层存证语义已明确；产品日志或履约确认调用本身仍不构成 TSD 合规记录 |

卖方的真实产品步骤始终以[支付宝 AI 按量付费接入指南](https://aipay.alipay.com/docs/ai-receive/MACHINE_PAY.html)为准。本仓库只负责说明这些步骤在 ACT 中的语义位置和验证责任。

CID 本轮黄色安全修订要求 Profile 不能信任任意来源的支付能力文件或端点：能力声明必须来自商户已声明的地址，并检查 PSP、端点和方法 Schema 的一致性；来源或关键字段校验失败时终止支付能力匹配。支付宝域名、接口和验签的产品级判断仍以官方接入文档为准，不能用 ACT 示例地址替代。
...
以下是完成版协议仍未规定、或需要产品映射确认的问题；Profile 只能记录映射，不能自行补造 Core Schema：

1. CID 双向协商中的 `currency` / `amount_currency`、`supported_methods` / `matched_methods` 应采用哪个正式 wire 名称，以及能力声明 Schema、版本和签名规则。
2. `Payment-Needed` 与独立 CID 交易确认对象的正式版本和签名规则；现有 Candidate 已允许引用，且不要求完整购物车。
3. 支付宝钱包开通/授权绑定与 `PSD-PMT-BND` 的准确边界，哪些属于产品账户操作而非协议消息。
4. 支付宝履约确认 API 属于 PSD 的产品回执还是业务回调；如何映射为独立的 `act:commerce:fulfillment-completed` 链下记录，以及是否/如何进入 ACT Trust Chain。
5. ACT 可选 `Payment-Validation` Header 与支付宝服务端验款结果如何映射；当前产品文档未定义同名 Header。
6. `intent_id`、`delegation_id`、商户订单号、`resource_id` 和支付宝交易号在 TSD 正式 wire Schema 中如何命名与条件必备。
7. DEL/AUP 所依赖的身份、连接、授权和密钥安全能力如何映射到 ADD 的 IAC 封装、状态查询和公开开发者路径。
8. 支付宝是否存在可公开映射到 TSD 信用关联验证的产品能力；没有公开产品事实前，本 Profile 不声明 `TSD-CRD-*` 支持。

这些问题同步记录在[修订追踪表](../../../../governance/revision-status.md)，后续应由协议决定、Profile 映射和沙箱证据共同闭环。
