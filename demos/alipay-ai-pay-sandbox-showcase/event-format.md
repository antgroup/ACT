...
> 状态：Preview / Non-normative。该格式只服务于演示编排与证据审查，不是 ACT 协议消息。

本格式只接受 `LIVE_SANDBOX` 和 `SANITIZED_REPLAY` 作为证据模式。Web 演示台内置的 `GUIDED_PREVIEW` 是说明性 UI 数据，校验器会拒绝将其作为支付证据。

证据文件使用 NDJSON，每行一个对象。成功闭环必须按顺序包含 11 个状态：

```json
{"sequence":1,"state":"CAPABILITY_NEGOTIATED","scenario":"SUCCESS","mode":"LIVE_SANDBOX","source":"buyer-agent-quickstart","environment":"SANDBOX","occurred_at":"2026-07-22T12:00:00Z","evidence_ref":"E2E-20260722-001#step-1","correlation_ref":"corr-sha256-a1b2","method_id":"alipay-ai-pay","psp_id":"alipay"}
```
...
| `source` | 产生该事件的 Quickstart、官方能力或验款步骤；不得为 Mock |
...
| `method_id` | `CAPABILITY_NEGOTIATED` 必填，表示双方选择的支付方法 |
| `request_ref`、`order_ref` | `ORDER_CONFIRMED` 必填，证明原请求与商户订单已绑定 |
...
| `validation_mapping` | 说明 ACT 候选 `Payment-Validation` 与支付宝 `payment.verify` 的产品映射 |
...
- `IDEMPOTENT_REPLAY`：沿成功状态返回既有验证与交付结果，但不得重复扣款或重复履约。

页面不显示任意扩展字段，并递归拒绝明显的密钥、令牌、完整 Proof、`client_session`、绑定码和密码字段。

不要写入业务响应全文。金额、订单和资源关联结论应保存在[端到端验证证据](../../docs/getting-started/end-to-end-evidence-template.md)中，事件仅保留引用。
