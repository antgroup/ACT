# Alipay AI Pay 与 ACT 四域映射

> 状态：July Preview / Non-normative  
> 映射基线：ACT 官网四域框架 + 支付宝 AI 付公开产品  
> 用途：说明产品如何组合 ACT 域，不定义或修改 ACT 正式协议

## 1. 先理解映射关系

Agent 支付和 AI 按量付费都不是一个新的 ACT 域，也不能各自与某一个域一一对应。它们是支付宝产品 Profile：复用 ACT 四域的跨产品语义，再补充支付宝字段、接口、签名、Skill/CLI 和产品错误。
...
因此，开发者的阅读顺序应当是：先按产品选择接入路径，再用本页确认该路径覆盖哪些 ACT 组件，最后进入支付宝官网完成真实产品接入。

## 2. 产品与四域总览

| 产品路径 | ADD | CID | PSD | TSD |
|---|---|---|---|---|
| Agent 支付（买方） | 捕获支付意图；未来委托模式使用 IAC | 保留商品、商户、订单和支付能力选择上下文 | 钱包绑定；首期用户确认支付；未来委托/自主支付 | 关联意图、订单、支付和履约证据 |
| AI 按量付费（卖方） | 消费买方上游授权上下文，本身不签发用户授权 | 描述收费资源、价格、订单和支持的支付方式 | 用 402 提出支付要求、接收凭证、验款和确认履约 | 记录支付完成与资源履约事件，支持后续核验 |
...
AI 按量付费是收费资源提供方的产品路径。它负责生成和验证支付要求，不负责替买方 Agent 决定是否有权支付。
