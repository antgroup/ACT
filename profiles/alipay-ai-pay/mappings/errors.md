...
本页的机器可读来源是 [`error-mapping.preview.json`](../schemas/error-mapping.preview.json)。仓库检查会验证每个产品错误都引用 [`error-catalog.json`](../../../specs/2.1/schemas/a402/error-catalog.json) 中存在的 Core Candidate 错误 ID；本页仅提供阅读说明。
...
These Profile-friendly names are aliases for developer guidance, not a second Core vocabulary. Implementations emitting ACT Candidate errors must use [`error-catalog.json`](../../../specs/2.1/schemas/a402/error-catalog.json); product-only actions such as `AUTHORIZE_WALLET` remain Profile extensions.
