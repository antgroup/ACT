...
The `protocol` and `method` envelope is now also an ACT 2.1 Candidate protocol fact. The fields in this table remain Alipay product serialization: their placement and requiredness are not automatically inherited by the provider-neutral [A402 Candidate Schema](../../../specs/2.1/schemas/a402/README.md). The Profile must explicitly adapt between the two contracts.
