...
> 协议映射基线：2026-08-03 完成修订的 PSD《支付服务域》  
...
ACT 官网当前按照四个域组织；本轮开源修订以 PSD 为主，其他域只保留支付链路所需的依赖映射：
...
1. `method_id` 命名空间、版本兼容、Header 字段分布和未知方法错误。
2. `Payment-Needed` 是否需要引用独立 CID 交易确认对象；协议已明确它可以携订单/资源标识且不要求完整购物车。
...
这些问题同步记录在[修订追踪表](../../../docs/project/revision-status.md)，后续应由协议决定、Profile 映射和沙箱证据共同闭环。
