...
> Warning: ACT names in this document are placeholders, not a published Schema.
...
The `protocol` and `method` property names are Alipay product serialization. ACT Core should not adopt that split without a separate protocol decision.
...
| `fulfillment.id` | no documented direct field | `PROTOCOL-PENDING` |
...
| FM-002 | Is `Payment-Proof` encoded using Base64 or Base64URL in every public client? | HTTP Binding |
