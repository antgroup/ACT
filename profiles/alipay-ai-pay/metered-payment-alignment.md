...
| Payment required | Provider returns HTTP 402 with `Payment-Needed` | AP-SRC-005, 006 | Payment Requirement | Core + HTTP Binding + Profile | `PUBLIC-FACT`; Core object `PROTOCOL-PENDING` | 402 conformance fixture |
...
| Request retry | Agent retries the original request with `Payment-Proof` | AP-SRC-004, 006 | Proof presentation | Core + HTTP Binding + Profile | `PUBLIC-FACT`; retry semantics `PROTOCOL-PENDING` | End-to-end sandbox |
...
| Business validation | Provider matches active status, amount, order and resource against the original bill | AP-SRC-006, 007 | Verification policy | Core responsibility + Profile mapping | `PUBLIC-FACT`; Core rules `PROTOCOL-PENDING` | Mismatch tests |
...
| Resource delivery | Provider returns the paid resource only after successful verification | AP-SRC-006 | Fulfillment | ACT Core/application | `PUBLIC-FACT`; lifecycle `PROTOCOL-PENDING` | End-to-end sandbox |
