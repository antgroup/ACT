...
> 状态：Active product-source baseline / Non-normative  
> 资料快照：2026-08-03  
> 用途：记录公开产品事实，为后续 Alipay AI Pay Profile 提供输入

本文不定义 ACT 正式消息。文中的支付宝字段、API 和命令来自公开官网、支付宝开放平台或官方开源仓库；未被公开资料直接回答的内容保持 Product Review，不阻塞 Profile Preview。
...
协议结构上，2026-08-03 完成版支付服务域把授权场景与支付接入方式分开：首期是 L1 `PSD-PAY-INS` 场景引用候选 `PSD-PAY-A402`，再由官方 Skill/CLI Binding 和 Alipay AI Pay Profile 落到支付宝产品。仓库转写仍为 Candidate / Non-normative。

两侧能力都不是 ACT 的独立域或单域实现。它们作为 Alipay AI Pay Product Profile 跨 ADD、CID、PSD、TSD 协作，具体见[双侧能力与 ACT 四域映射](../mappings/domains.md)。
...
当前官方 Skill/CLI 是工作流级 Binding：在 402 场景中可以保存账单和原始请求、发起或查询支付、携带 Proof 恢复请求并返回资源。ACT 接入不应假设宿主 Agent 必须直接获得完整 Proof。详细核对见[官方 Skill/CLI 行为审计](audits/2026-07-21-skill-cli.md)。
...
官网当前还推荐使用以下产品接入 Skill 辅助现有 API 完成集成、沙箱测试和签约入驻：

```bash
npx -y @alipay/alipay-aipay@latest install
```

该包面向卖方产品接入，不等同于买方 `@alipay/agent-payment`，也不是新的 ACT 支付协议。
...
组件级映射以[四域映射文档](../mappings/domains.md)为准。以下工作语义表保留用于字段和 Profile 讨论。
...
| Payment Intent | Payment Skill 支付意图和用户确认 | Core + Profile | L1 Candidate mapped |
| Payment Requirement | `Payment-Needed` | Core + HTTP Binding + Profile | A402 Candidate + Product Preview Schema |
| Payment Proof | `Payment-Proof` | Core + HTTP Binding + Profile | A402 Candidate + Product Preview Schema |
| Proof Verification | `alipay.aipay.agent.payment.verify` | Alipay Profile | 产品事实待复核 |
| Fulfillment Receipt | `alipay.aipay.agent.fulfillment.confirm` | Core + Profile | Candidate boundary fixed; Product mapping review |
| Recovery Action | 支付宝错误码、重试或重新支付 | Core + Profile | Candidate vocabulary fixed; Product mapping review |
...
这些问题在确认前进入[修订追踪](../../../docs/project/revision-status.md)，不能由示例代码自行假设答案。它们默认只阻塞对应的产品能力声明或 `Sandbox Verified`，不阻塞 ACT Candidate 和 Profile Preview；若公开资料足以支持安全下限，Profile 会明确记录该下限。
