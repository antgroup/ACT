...
> **状态：Candidate Working Draft / Non-normative Scenario Guide**  
> **用于理解跨域组合，不是独立协议组件、正式实现规范或 Conformance 证据。**  
> 官方发布入口最后核对：2026-08-10（UTC+8）

本文把 ADD、CID、PSD 和 TSD 组合成端到端业务场景，帮助开发者判断何时需要 IAC、采用哪一种支付授权级别，以及何时异步形成可信事件。协议事实源是 ACT Protocol 官网的[典型场景与业务流程](https://www.act-protocol.com/documentation/scenarios)。

该来源没有黄色高亮修改点，因此本次属于开源仓库首次补齐场景基线，不记作新的协议决策。若场景文档的组件清单或示意图与完成版域正文冲突，以对应域正文为准；例如场景表没有列出 `PSD-PAY-A402`，但完成版[支付服务域](domains/payment-services.md)已经把它定义为可被 INS、DEL、AUP 引用的独立接入组件。
...
- TSD 当前只有 Candidate 语义，没有本仓库可运行的 ACT Trust Chain 或信用服务。产品日志和沙箱证据可以作为未来映射输入，但不得声明已实现 TSD。
