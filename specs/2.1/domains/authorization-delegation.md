...
本域以 ACT Protocol 官网的[委托授权域](https://www.act-protocol.com/documentation/delegation)为协议事实源。本仓库只提供 Candidate / Non-normative 快照，不把官网内容或历史同步结果升级为已完成治理的正式规范。
...
来源将 ISR 定义为数据字典，而不是冻结的 wire Schema：它规定名称、类型和语义，但不统一规定本表全部字段的存在性。场景规则、IAC 签发规则和下游组件负责进一步约束。
...
> **历史修订证据：** 维护稿只对 `price_deviation_tolerance` 和 `price_deviation_action` 中的 `deviation` 作黄色标记。本 Candidate 将它们作为当前字段名保留；该标记只解释快照演进，不是当前公开事实源。
...
来源明确了数据字典、存在性和处理语义，但没有发布可直接宣称正式兼容的 ISR/IAC JSON Schema、封装版本、算法套件、状态查询协议、事件 Schema 或标准错误对象。本仓库不从示例或描述推造这些机器契约；相关事项见[修订状态](../revision-status.md)。
