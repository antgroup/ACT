...
本文件是 `PSD-PAY-A402` 的候选协议入口。在候选架构中，A402 是与支付场景解耦的接入协议组件；具体 HTTP 序列化实现同时受跨产品的[HTTP A402 Binding](../../../integrations/bindings/http-a402/README.md)和所选 Product Profile 约束。
...
支付宝的具体 `out_trade_no`、`resource_id`、`seller_signature`、`seller_id`、`service_id`、`payment_proof`、`trade_no`、`client_session` 等字段属于[Alipay AI Pay Profile](../../../integrations/profiles/alipay-ai-pay/README.md)，不直接冻结为渠道中立 Core Schema。
...
上述内容是[Candidate 工程决议](../../../governance/decisions/candidate-machine-contract-resolution-2026-08-03.md)，不是已完成公开 DWG 治理的 Stable 结论。

### 5.1 基础载荷字段清单

完成版支付服务域给出以下跨方法字段语义。Candidate 工程层已经在[机器契约](schemas/README.md)中冻结三类 Header 的字段分布、必填性和格式，供开源实现与测试互操作；正式治理仍可通过新版本替代，不能把当前 Schema 宣称为 Stable。
...
完整算法和保存下限见[Candidate 工程决议](../../../governance/decisions/candidate-machine-contract-resolution-2026-08-03.md#3-请求指纹和防重放规则)。
...
实现不得创造反向转移或从终态重新进入处理中状态。Candidate 触发事件已经固定为 `PAYMENT_NEEDED_ISSUED`、`PAYMENT_VALIDATED`、`PAYMENT_TERMINATED`、`DELIVERY_COMMITTED`、`COMPENSATION_COMPLETED`、`FULFILLMENT_CONFIRMED` 和 `TRADE_TERMINATED`；允许关系由 [`transaction-state.schema.json`](schemas/transaction-state.schema.json)及 fixtures 执行检查。产品状态仍由 Profile 映射，超时本身不得被当作支付失败。

## 9. 幂等与错误恢复

完成版支付服务域定义错误的通用语义类别和恢复方向。Candidate 机器层将它们固定为 `error_id`、`category`、`phase`、`retryability`、`valid_next_actions` 和 `message`；完整代码见 [`error-catalog.json`](schemas/error-catalog.json)：
...
- 官网尚未完整发布的 A402 Candidate 差异：[修订状态](../revision-status.md)
