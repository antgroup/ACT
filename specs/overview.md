...
| ADD-IAC-ISS：意图授权凭证的签发 | 规范将结构化意图转化为跨域可验证的密码学授权凭证（IAC）的签发与签名机制 |
| ADD-IAC-LCM：意图授权凭证生命周期管理 | 定义 IAC 从生效、挂起、恢复到作废、过期的全生命周期状态流转机制 |
...
| 争议中心 | Dispute Center | 为 ACT 协议生态中的参与方提供争议提交、调解与裁决服务的机构或平台 |
