...
- 用户意图的获取及确认过程，以及从原始意图输入到意图结构化规则的转换与委托人确认流程；
- 定义用户意图结构化规则，包括字段体系与语义约束；
...
### 意图结构化规则 Intent Structured Rule, ISR

将委托人的商业意图转化后的机器可读约束规则，定义本次委托的约束边界。

> 注：考虑到隐私保护与安全隔离，并非 ISR 中的所有字段都需要参与最终 IAC 的签名计算。
...
| 协议组件 | 组件描述 |
|---------|---------|
| `ADD-INT-EAC`：用户意图表达与确认 | 规范委托人向智能体表达自然语言意图，以及智能体解析并获得用户明确确认的交互流程。 |
| `ADD-INT-ISR`：意图结构化表达规则 | 将用户的商业意图转化为语义严谨、包含基本与扩展字段的机器可读结构化约束规则。 |
| `ADD-IAC-ISS`：意图授权凭证的签发 | 规范将结构化意图转化为跨域可验证的密码学授权凭证的签发与签名机制。 |
...
- `ADD-INT-EAC`：智能体获取并解析用户的购买意图；
- `ADD-INT-ISR`：智能体使用意图结构化表达规则对用户意图进行表达。
...
- `ADD-INT-EAC`：平台型智能体获取并解析用户的定向购买意图；
- `ADD-INT-ISR`：智能体使用意图结构化表达规则对用户意图进行表达；
...
- `ADD-INT-EAC`：专属型智能体获取并解析用户的定向购买意图；
- `ADD-INT-ISR`：智能体使用意图结构化表达规则对用户意图进行表达；
...
- `ADD-INT-EAC`：智能体获取并解析复杂的任务目标与约束边界；
- `ADD-INT-ISR`：智能体使用意图结构化表达规则对用户意图进行表达；
...
2. **意图解析与澄清**：智能体对委托人意图进行深度解析，将自然语言表述映射为本协议定义的意图结构化规则（ISR）草稿。在映射过程中，智能体需评估是否需要向委托人进行意图澄清（例如：当委托人表述模糊、核心约束字段缺失，或预算不合理时）。智能体可与委托人进行多轮交互以逐步完善 ISR 草稿；具体的交互形式（如文本、语音、UI 卡片等）及澄清触发的推理逻辑，属于应用层智能体内部实现，不在本协议规范范围内。
...
### 5.2 ADD-INT-ISR：意图结构化表达规则

#### 5.2.1 概述

意图的结构化表达规则（Intent Structured Rule）旨在将用户通过自然语言描述的购物或商业意图，转化为语义严谨、机器可读的结构化表达方式。这种结构化表达可用于：

- 对用户意图的澄清与确认；
- 用户意图授权凭证的签发；
- 执行阶段意图一致性的比对核查；
- 意图上下文在智能体与下游商户/服务方之间的传递。

在规则结构设计上，本协议对 ISR 采用"基本字段 + 扩展字段"的分层设计。基本字段规范了涉及身份、授权边界和时效性等全局通用的参数；扩展字段则提供了一套处理机制，用以承载不同垂直行业或特定商户的复杂商业约束。

#### 5.2.2 基本字段

以下字段为结构化意图的基本字段，构成了对用户意图进行结构化表达的核心要素。

| 字段名 | 类型 | 存在性 | 说明 |
|--------|------|--------|------|
| `user_intent_raw` | string | 必填 | 用户原始意图文本 |
...
### 5.3 ADD-IAC-ISS：用户意图授权凭证的签发

#### 5.3.1 概述

用户意图授权凭证签发（Intent Authorization Credential Issuance）规范了意图授权凭证的业务签发流程、封装格式与签名计算规范。本组件界定了从意图规则（ISR）转化为跨域可验证的密码学凭证的过程。
...
### 5.4 ADD-IAC-LCM：用户意图授权凭证生命周期管理

#### 5.4.1 概述

用户意图授权凭证生命周期管理（Intent Authorization Credential Life Cycle Management）定义了 IAC 的标准状态机。
...
| 事件类型标识 | 触发时机 |
|-------------|---------|
| `act:delegation:intent-created` | `ADD-INT-EAC` 确认完成 |
