...
协议修订和开源工程重构并行推进，但使用不同准入条件：

- 协议修订负责确定正式语义、组件、状态和字段。
...
大会版本的详细范围和发布门槛见[发布章程](06-bund-release-charter.md)。关键日期如下：
...
- Common Message 候选模型。
- Payment Requirement 候选语义。
- Payment Proof 与验证责任。
- 生命周期、回调、错误恢复和下一步动作。
- Agent Discovery/Profile 候选模型。
- Core 与 Extension 边界。
