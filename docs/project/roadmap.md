...
> 状态：Active delivery roadmap / Non-normative；最后收敛：2026-08-03

## 1. 总体策略

协议修订和开源工程重构并行推进，当前采用 PSD-first：

- 2026-08-03 完成版《支付服务域》是完整协议事实源，v2.1 修订方案只作为演进背景。
- 公开 2.0 目录不恢复；有效 PSD 语义在 2.1 Candidate 中显式维护。
- 近期协议工作聚焦 PMT-BND、AGT-SUB、INS/L1、DEL/L2、AUP/L3 和独立 A402。
- ADD、CID、TSD 只维护支付链路需要的引用边界，不阻塞首期一键接入。
...
大会版本的详细范围和发布门槛见[发布章程](releases/2026-bund-charter.md)。关键日期如下：
...
## 1.2 当前落地状态（2026-08-03）

| 工作项 | 状态 | 证据/入口 |
|---|---|---|
| 当前项目框架成为唯一结构事实来源 | Done | [ACT 项目框架](../architecture/README.md) |
| 支付宝 AI 付产品级 Quickstart 入口 | Done | [支付宝 AI 付 Quickstart](../../quickstarts/alipay/README.md) |
| Agent 支付、AI 按量付费、端到端 402 中文主路径 | Done | [Quickstart 导航](../../quickstarts/README.md) |
...
| Buyer/Seller Demo Adapter 与证据导出工具 | Done | `demos/alipay-ai-pay-sandbox-showcase/` |
| 完成版 PSD 协议源、六组件和 A402 语义 | Done as Candidate | `specs/2.1/`、`scripts/check_repository.py` |
| A402 三类载荷 Schema、错误词典、fixtures 与 validator | Done as Candidate | `specs/2.1/schemas/a402/`、`scripts/validate_a402_contract.py` |
| Alipay Profile Preview 与产品字段机器契约 | Done for Preview；产品复核项保留 | `profiles/alipay-ai-pay/` |
| 卖方凭证配置后单命令启动 | Done | `quickstarts/alipay/metered-rest-provider/run.sh` |
| 官方沙箱端到端证据 | Optional validation claim pending | 只阻塞 `alipay-sandbox-verified` |
| ACT Candidate / Profile Preview / Sandbox Verified 分级门禁 | Done | `docs/project/releases/release-readiness.json` |
...
| M6 Candidate Publication | Candidate/Profile 内容与公开发布治理门禁完成 | M4、法务、安全、公开远端 |
