...
> 状态：Draft / Non-normative。正式成员名单、投票权和对外承诺需在首次公开发布前由项目发起方确认。

本文档描述 ACT Protocol 拟采用的治理结构、决策流程和角色职责。在正式确认前，它用于公开讨论，不授予任何组织或个人治理席位。
...
正式 TSC 成员名单尚待项目发起方确认。确认后应公开记录成员、代表、任期、投票权和利益冲突披露；在此之前不以公司名称或历史参与关系推定正式席位。
...
A402 Candidate 的具体评审可使用项目级[A402 决策执行指南](docs/project/decisions/a402-review-guide.md)记录选项、证据和 ADR 输入。该指南不授予投票权，也不改变本文件尚待确认的正式成员与决策规则。
...
- CLA（贡献者许可协议）是否启用及其公开签署机制仍待维护者确认；机制发布前不应把它描述为已可执行的贡献前置条件
- 项目保留统一的品牌和商标使用权
