...
> 状态：Provisional / Non-normative。本文作为 ACT 2.1 Candidate 的公开讨论和仓库维护流程生效；正式 TSC/DWG 成员、投票权和 Stable/Recommendation 发布权尚未建立。

本文档描述 ACT Protocol 的临时维护流程与目标治理结构。当前工作负责人见 [`MAINTAINERS.md`](MAINTAINERS.md)；这些分工允许维护 Candidate 内容和仓库工程，但不自动授予 TSC 席位或正式规范发布权。
...
### 2.1 开放性（Openness）
公开后的协议设计与演进过程默认可追溯，任何人均可参与讨论与贡献；安全事件、个人信息和依法不能公开的材料除外。

### 2.2 中立性（Neutrality）
ACT Protocol 当前由 Ant Group 发起并持有本仓库版权，协议设计保持产品中立，并以逐步扩大社区参与和独立实现验证为治理目标。版权归属不赋予任何支付宝产品事实定义 ACT Core 语义的权力。
...
正式 TSC 成员名单尚待项目发起方确认。确认后应公开记录成员、代表、任期、投票权和利益冲突披露；在此之前不以公司名称、工作负责人或历史参与关系推定正式席位，也不得通过 TSC 投票发布 Stable/Recommendation。
...
A402 Candidate 的具体评审可使用项目级[A402 决策执行指南](governance/decisions/a402-review-guide.md)记录选项、证据和 ADR 输入。该指南不授予投票权，也不改变本文件尚待确认的正式成员与决策规则。
...
> [!IMPORTANT]
> 规范目录使用的 **Candidate Working Draft / Non-normative** 只表示内容正在公开候选评审，不表示已经进入 SEP **Candidate（RC）** 阶段。SEP Candidate 仍必须满足两个独立实现验证和本节的阶段转换条件。
...
- 贡献按 [LICENSE](LICENSE) 中的文件类型分配发布：文档当前指向 `CC-BY-4.0`，代码、Schema、测试和自动化当前指向 `Apache-2.0`；版权主体和最终分配在首次正式公开发布前仍需维护者或法务确认
- 当前不启用额外贡献协议或 DCO sign-off 前置；贡献者仍须确认有权提交，并接受仓库对应的出站许可证
- 项目保留统一的品牌和商标使用权
...
- 正式治理、社区和安全联系方式尚待项目发起方确认；不得使用未验证的占位邮箱对外承诺响应。
