...
> 范围：L1 `PSD-PAY-INS` + 候选 `PSD-PAY-A402` + Alipay AI Pay Profile
...
```bash
npm --prefix quickstarts/alipay/end-to-end-402 run sandbox:preflight
```

该命令不读取或输出密钥内容，不创建订单、不发起支付。只有输出 `local-preflight-passed` 后，才进入官网沙箱订单与用户逐笔授权步骤；它本身不构成任何支付或兼容性证据。

2026-08-03 的当前工作区检查记录位于 [`2026-08-03-sandbox-preflight.json`](../../../docs/project/releases/evidence/2026-08-03-sandbox-preflight.json)：npm、官方 `alipay-bot`、JDK 和 Maven 已就绪；被 Git 忽略的卖方 `.env` 已建立，但应用、商户、服务字段仍含占位值，应用私钥和支付宝公钥也未配置。因此本次没有创建订单或发起支付，也不能声明真实 Sandbox E2E 已通过。
...
本仓库不实现或复制支付宝 Sandbox、测试账号、沙箱 App、控制台和密钥分配。以上产品操作统一使用 [AIPay 官网](https://aipay.alipay.com/callpay)及其接入文档。真实 E2E 只用于授予 `Alipay AI Pay Sandbox Verified` 声明，不是发布 ACT Candidate 或 Profile Preview 的必要条件。
...
产品操作按官网完成后，使用[端到端证据模板](../../../docs/getting-started/end-to-end-evidence-template.md)记录：

- ACT、Alipay Profile、Quickstart 和官方 Skill/CLI 的精确版本；
...
Sandbox Showcase 的脱敏事件格式见 [Demo 事件格式](../../../demos/alipay-ai-pay-sandbox-showcase/event-format.md)。Replay 只能来自已通过的官网沙箱链路。
