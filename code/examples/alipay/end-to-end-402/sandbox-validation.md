...
2026-08-03 的当前工作区检查记录位于 [`2026-08-03-sandbox-preflight.json`](../../../../governance/releases/evidence/2026-08-03-sandbox-preflight.json)：npm、官方 `alipay-bot`、JDK 和 Maven 已就绪；被 Git 忽略的卖方 `.env` 已建立，但应用、商户、服务字段仍含占位值，应用私钥和支付宝公钥也未配置。因此本次没有创建订单或发起支付，也不能声明真实 Sandbox E2E 已通过。

配置入口：

```bash
cd code/examples/alipay/metered-rest-provider
./run.sh init
```
...
Sandbox Showcase 的脱敏事件格式见 [Demo 事件格式](../../../web-client/alipay-ai-pay-showcase/event-format.md)。Replay 只能来自已通过的官网沙箱链路。
