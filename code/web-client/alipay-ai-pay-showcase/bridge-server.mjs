...
const inheritedContextFields = [
  "method_id", "method_version", "psp_id", "endpoint_ref", "method_schema_ref",
  "capability_source_ref", "capability_source_validated", "commerce_confirmation_ref",
  "request_ref", "request_fingerprint", "http_method", "order_ref", "proof_ref",
  "transaction_ref", "amount", "currency", "resource_id", "profile_mapping",
];
  ...
  const server = createServer(async (request, response) => {
    ...
    if (request.method === "POST" && url.pathname === "/events") {
      try {
        ...
        const event = normalizeEvent(input, events.length, scenario, events);
...
function normalizeEvent(input, index, scenario, priorEvents) {
  const event = {
    sequence: index + 1,
    mode: "LIVE_SANDBOX",
    environment: "SANDBOX",
  ...
  for (const key of inheritedContextFields) {
    for (let priorIndex = priorEvents.length - 1; priorIndex >= 0; priorIndex -= 1) {
      if (priorEvents[priorIndex][key] !== undefined) {
        event[key] = priorEvents[priorIndex][key];
        break;
      }
    }
  }
