    ...
    for (const [index, state] of states.entries()) {
      const response = await fetch(`${base}/events`, {
        ...
        body: JSON.stringify({
          ...
          ...(state === "CAPABILITY_NEGOTIATED" ? capabilityFacts : {}),
          ...(state === "ORDER_CONFIRMED" ? { commerce_confirmation_ref: chainFacts.commerce_confirmation_ref } : {}),
          ...(state === "RESOURCE_REQUESTED" ? { request_ref: chainFacts.request_ref, http_method: "GET" } : {}),
          ...(state === "PAYMENT_REQUIRED" ? {
            request_fingerprint: chainFacts.request_fingerprint,
            order_ref: chainFacts.order_ref,
            resource_id: chainFacts.resource_id,
            amount: chainFacts.amount,
            currency: chainFacts.currency,
            profile_mapping: chainFacts.profile_mapping,
          } : {}),
          ...(state === "PAYMENT_RESULT_RECEIVED" ? {
            transaction_ref: chainFacts.transaction_ref,
          ...
          ...(state === "FULFILLMENT_CONFIRMED" ? {
            fulfillment_ref: "fulfillment-sha256-a1b2",
